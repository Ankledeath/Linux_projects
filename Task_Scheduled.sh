...
add_task() {
        ...
        read -p "Enter the schedule (hourly, daily, weekley): " schedule
...
remove_task() {
        clear
        read -p "Enter the command or scipt to be removed: " command

        # Remove the task from the crontab
        crontab -l | grep -v "$command" | crontab -

        echo "Task removed succefully."
        echo
